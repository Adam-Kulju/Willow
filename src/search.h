    ...
    for (depth = 1;; depth++)
    {
        ...
        while (abs(evl) != TIMEOUT && (evl <= alpha || evl >= beta))
        {
            ...
            else if (evl >= beta) // If we fail high, widen the window
            {
                ...
                if (isprint)
                {
                    printf("info depth %i seldepth %i score cp %i nodes %lu time %li pv %s\n", depth, maxdepth, beta, nodes, (long int)((float)clock() - start_time) * 1000 / CLOCKS_PER_SEC, conv(thread_info->currentmove, temp));
                }
...
void start_search(struct board_info *board, struct movelist *movelst, float maxtime, int *key, bool color, ThreadInfo *thread_info, int numThreads){
    ...
    *thread_info->key = *key;
    ...
    for (int i = 0; i < numThreads-1; i++){
        threads.emplace_back(iid_time, thread_infos[i].board, thread_infos[i].movelst, thread_infos[i].key, color, false, false, nullmove, &thread_infos[i]);
    }
