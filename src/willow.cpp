                         ...
                         // the buffer in this case)
{
  ...
  if ((buffer == NULL) ||
      (count <
       1)) // if we have neither a buffer or any space to put it, return null
  {
    result = NULL;
  } else {
    result = fgets(buffer, count, stdin);
    if (result ==
        NULL) // a check for EOF (indicating that whatever is running Willow (OB
              // etc.) has been killed and it's time to exit)
    {
      free(TT);
      exit(0);
    }
...
perft(int depth, struct board_info *board, bool color, bool first,
                               ...
                               // displaying results for each move at the root.
{
       ...
       i++) // Loop through all of the moves, skipping illegal ones.
  {
    ...
    long long unsigned int b =
        perft(depth - 1, &board2, color ^ 1, false, thread_info);
    if (first) {
      char temp[6];
...
void move_uci(char *command, int i, struct board_info *board,
              struct movelist *movelst, int *key, bool *color,
  ...
  while (command[i] != '\0' && command[i] != '\n') {
    ...
    bool iscap = (temp.flags == 0xC ||
                  board->board[temp.move &
                               0xFF]); // captures reset the halfmove clock for
...
int com_uci(struct board_info *board, struct movelist *movelst, int *key,
            bool *color,
            ThreadInfo *thread_info) // handles various UCI options.
{ // assumes board+movelist have been already set up under wraps
  ...
  if (strstr(command, "setoption name Hash value")) {
    ...
    int size = 0;
    while (sizeof(struct ttentry) * ((size_t)1 << size) <=
           target) // Set the hash to 2^n entires where n is the largest number
                   // that satisfies entry size * 2^n <= the option that was set
    {
      size++;
    }
    size--;
  ...
  if (strstr(command, "go")) {
    ...
    if (strstr(command, "infinite")) {
      ...
      coldturkey = time;
                    ...
                    "wtime")) // the "wtime" command indicates that Willow is
                              // playing a game currently. In this case, we use
                              // information about our time in order to get an
                              // optimal thinking time for the move before
                              // searching.
    {
      int k = 0;
      int movestogo = -1;
      if (strstr(command, "movestogo")) {
        int m = 0;
        while (command[m] != 'm' || command[m + 1] != 'o' ||
               command[m + 2] != 'v' || command[m + 3] != 'e' ||
               command[m + 4] != 's') {
          m++;
        }
      ...
      if (milltime < 1) {
        ...
        if (strstr(command, "winc")) {
          ...
          if (*color == BLACK) {
            ...
            while (isblank(command[k])) {
              k++;
            } // if we're playing as black, we need to skip again to past
              // "binc". it's usually the same, but eventually it might not be
              // so always good to plan ahead.
          }
          milltime = atoi(&command[k]);
          if (time + ((float)milltime / 1000 * 4) <
              coldturkey) { // if you have at least four increments left over,
                            // it's safe to add half the increment to your move.
                                   ...
                                   // and nodes per second.
{
  ...
  printf("Bench: %lu nodes %i nps\n", t,
         (int)(t / ((clock() - start) / (float)CLOCKS_PER_SEC)));
  return 1;
}
  ...
  if (argc > 1) {
    ...
    if (!strcmp(argv[1], "bench")) {
      ...
      printf("%llu nodes %i nps\n", a,
             (int)(a / ((clock() - start) / (float)CLOCKS_PER_SEC)));
      exit(0);
    }
  }
