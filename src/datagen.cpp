...
float game(const std::string &filename, ThreadInfo *thread_info) {
  ...
  while (!game_end && res == 0.5 && key < 900 && !checkdraw1(&board) &&
         !checkdraw2(movelst, &key)) {
    char fen[100];
    ...
    bool isnoisy = (thread_info->currentmove.flags == 0xC ||
                    thread_info->currentmove.flags == 0x7 ||
                    board.board[thread_info->currentmove.move & 0xFF]) &&
                   !(thread_info->currentmove.flags == 8);
