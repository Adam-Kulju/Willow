...
void pawnmoves(struct board_info *board, struct list *list, int *key,
  ...
  if (!color) {
    ...
    if (!board->board[diff]) { // if the square in front is empty, go forwards
      ...
      // Brings the current position to the beginning of the integer; if our
      // move was h8-h7, this sets it up as 0x7700 so we can ad the move to h7
      // (0x7767)
      if (((diff) >> 4) == 7) { // we have a promotion, so add all four
                                // promotions to the movelist.
        list[*key].move.move = t + diff,
        list[(*key)++].move.flags =
            4; // 4 = knight, 5 = bishop, 6 = rook, 7 = queen
        ...
        if ((pos >> 4) == 1 && !board->board[diff + NORTH]) {
          ...
          list[*key].move.move = t + diff + NORTH,
          list[(*key)++].move.flags = 0;
        }
      }
    }
    diff++;
    if (!(diff & 0x88) && board->board[diff] &&
        (board->board[diff] & 1)) { // capture to the right
    ...
    diff -= 2;
    if (!(diff & 0x88) && board->board[diff] &&
        (board->board[diff] & 1)) { // capture left
      unsigned short int t = pos << 8;
      if (((diff) >> 4) == 7) { // we have a promotion.
        list[*key].move.move = t + diff, list[(*key)++].move.flags = 4;
        list[*key].move.move = t + diff, list[(*key)++].move.flags = 5;
    ...
    if (!board->board[diff]) { // if the square in front is empty, go forwards
      ...
      if (((diff) >> 4) == 0) { // we have a promotion.
        ...
        list[*key].move.move = t + diff,
        list[(*key)++].move.flags =
            0; // go forwards and see if we can go forwards again

        if ((pos >> 4) == 6 && !board->board[diff + SOUTH]) {

          list[*key].move.move = t + diff + SOUTH,
          list[(*key)++].move.flags = 0;
        }
      }
    }
...
int movegen(struct board_info *board, struct list *list, bool color,
            bool incheck) { // a full pseudolegal move gen
  int key = 0;
  for (unsigned char i = 0; i < 0x80; i++) {
    ...
    if (board->board[i] && (board->board[i] & 1) == color) {
      ...
      if (piecetype == -1) { // handle pawns
        pawnmoves(board, list, &key, i, color);
      }

      else {
        for (unsigned char dir = 0; dir < vectors[piecetype]; dir++) {
          // Go in the directions that the piece can move, until we hit another
          // piece or the edge of the board; then move on to the next direction.
          for (int pos = i;;) {
            pos += vector[piecetype][dir];
            if ((pos & 0x88) ||
                (board->board[pos] && (board->board[pos] & 1) == color)) {
              break;
            }
  ...
  if (board->epsquare) {
    // handle special moves; en passant and castling. If the en passant square
    // (triggered by opponent moving a pawn two squares) is beside our pawn we
    // can take it en passant.
    if (!((board->epsquare + 1) & 0x88) &&
        board->board[board->epsquare + 1] == WPAWN + color) {
      if (!color) {
        list[key].move.move =
            ((board->epsquare + 1) << 8) + board->epsquare + NORTH,
        list[key++].move.flags = 0xC;
      } else {
        list[key].move.move =
            ((board->epsquare + 1) << 8) + board->epsquare + SOUTH,
        list[key++].move.flags = 0xC;
      }
    }
    if (!((board->epsquare - 1) & 0x88) &&
...
bool static_exchange_evaluation(struct board_info *board, struct move mve,
                                bool color, int threshold) {
  // Does a static exchange evaluation of the current position.

  if (mve.flags &&
      mve.flags >> 2 != 1) { // castling and en passant both come out neutral;
                             // thus we can return immediately.
    return (threshold <= 0);
  }
  int to = mve.move & 0xFF;

  int gain = SEEVALUES[board->board[mve.move & 0xFF] >>
                       1]; // what you get from taking the piece
  int risk = SEEVALUES[board->board[mve.move >> 8] >>
                       1]; // what you lose if your piece gets taken!
  ...
  int totalgain =
      gain -
      threshold; // how much you have gained after the first capture you made.
  ...
  while (totalgain - risk < 0) { // an example with RxB BxR RxB
    ...
    totalgain -=
        risk; // first we subtract the value of the piece that you just had
              // taken (since this is from opponent's point of view). in our RXB
              // case, that was a bishop, so we're now at -200.
    risk =
        SEEVALUES[i]; // if the bishop gets taken, he loses 300 (a minor piece).
    if (totalgain + risk <
        0) { // if the piece he took with was low enough value that you can't
             // get the material back by taking it (2 pawns for a bishop),
             // return. not true here.
      return false;
    }
    board2.board[attacker_pos] =
        BLANK; // remove the piece to account for batteries.
               // and yeah this fails to discovered checks, but a discovered
               // check will quickly cause a beta cutoff anyways so no big deal.

    i = get_cheapest_attacker(
        &board2, to, &attacker_pos,
        color);    // now we look at it from our point of view.
    if (i == 10) { // if we have no attackers, it is losing, since the condition
                   // in the while loop guarantees at least equal if we do
                   // something like BxB.
      return false;
    }
  ...
  // RxB is not winning in and of itself, so we get the smallest attacker which
  // turns out to be Q. this sets totalgain to -300, but risk to 900. we can
  // take it, and it's with a pawn. Total gain becomes 600, risk becomes 100,
  // and we return true.
  return true;
}
...
int see(struct board_info *board, struct move mve, bool color, int threshold) {
  // Orders captures.
  if (mve.flags ==
      0xC) // En passant is rated just above all the other PxP captures.
  {
    return 2000000 + 100 * 10;
  }

  int attacker = SEEVALUES[board->board[(mve.move >> 8)] >> 1],
      victim = SEEVALUES[board->board[(mve.move & 0xFF)] >> 1];
  // Get the values of the victim and the attacker

  int v = 2000000; // ciekce's magic v!

  if (victim - attacker < threshold &&
      !static_exchange_evaluation(board, mve, color, threshold)) {
    // If the piece we're trying to capture is worth less than the attacker, and
    // a static exchange evaluation also verifies the move looks bad, order it
    // at the very end
    v = -v;
  }
...
int movescore(struct board_info *board, struct movelist *movelst, int *key,
              struct list *list, int depth, bool color, char type, int movelen,
              int threshold, ThreadInfo *thread_info, ttentry entry,
              bool score_quiets) {
  ...
  while (i < movelen) {
    ...
    else if (list[i].move.flags ==
             7) // Queen promotions are almost certainly good for us, so order
                // them right below the TT move.
    {
      list[i].eval +=
          5000000 + SEEVALUES[board->board[(list[i].move.move & 0xFF)] >> 1];
    } else if (board->board[list[i].move.move & 0xFF] &&
               list[i].move.flags != 8) // Score captures with MVV-LVA and SEE.
    {
      list[i].eval = see(board, list[i].move, color, threshold) +
                     thread_info->CAPHIST[color][list[i].move.move >> 8]
                                         [list[i].move.move & 0xFF];
    } else if (!score_quiets) {
      i++;
      continue;
    } else if (ismatch(list[i].move,
                       thread_info->KILLERTABLE[depth][0])) // Killer moves
    {
      list[i].eval += 199;
    }
    ...
    {
      list[i].eval = thread_info->HISTORYTABLE[color][list[i].move.move >> 8]
                                              [list[i].move.move & 0xFF];
      if (isreply) {
        list[i].eval +=
            thread_info->CONTHIST[lastpiecetype][lastpiecedest]
                                 [board->board[list[i].move.move >> 8] - 2]
                                 [list[i].move.move & 0xFF];
      }
      if (depth > 1 && depth < 90 && movelst[*key - 2].piecetype != -1) {
        list[i].eval +=
            thread_info->CONTHIST[movelst[*key - 2].piecetype]
                                 [movelst[*key - 2].move.move & 0xFF]
                                 [board->board[list[i].move.move >> 8] - 2]
                                 [list[i].move.move & 0xFF];
      }
      if (depth > 3 && depth < 90 && movelst[*key - 4].piecetype != -1) {
        list[i].eval +=
            thread_info->CONTHIST[movelst[*key - 4].piecetype]
                                 [movelst[*key - 4].move.move & 0xFF]
                                 [board->board[list[i].move.move >> 8] - 2]
                                 [list[i].move.move & 0xFF];
      }
    }
